# Python Tutorial Guide

| Topic | Description |
|-------|-------------|
| [Output](pages/Output.md) | Print text and numbers, perform arithmetic, and use f-strings. |
| [Comments](pages/Comments.md) | Add notes and comment out Python code. |
| [Variables](pages/Variables.md) | Store and manage data values. |
| [Data Types](pages/Data-Types.md) | Store different types of data, each with its own behavior. |
| [Numbers](pages/Numbers.md) | Work with integers, floats, and other numeric values. |
| [Casting](pages/Casting.md) | Convert values between types using functions like `int()`, `float()`, and `str()`. |
| [Strings](pages/Strings.md) | Create, modify, and work with text data using Python string features. |
| [Booleans](pages/Booleans.md) | Represent True/False values and evaluate conditions.  |
| [Operators](pages/Operators.md) | Perform arithmetic, assignment, comparison, logical, identity, membership and bitwise operators. |
